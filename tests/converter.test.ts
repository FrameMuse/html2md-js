...
describe("paragraphs", () => {
  test("simple paragraph", () => {
    expect(htmlToMd(el("<p>Hello World</p>"))).toBe("Hello World")
  })

  test("two paragraphs", () => {
    expect(htmlToMd(el("<p>First</p><p>Second</p>"))).toBe("First\n\nSecond")
  })

  test("empty paragraph", () => {
    expect(htmlToMd(el("<p></p>"))).toBe("")
  })
})

describe("headings", () => {
  test("h1", () => {
    expect(htmlToMd(el("<h1>Title</h1>"))).toBe("# Title")
  })

  test("h2", () => {
    expect(htmlToMd(el("<h2>Section</h2>"))).toBe("## Section")
  })

  test("h3", () => {
    expect(htmlToMd(el("<h3>Sub</h3>"))).toBe("### Sub")
  })
})

describe("bold and italic", () => {
  test("bold via strong", () => {
    expect(htmlToMd(el("<strong>bold</strong>"))).toBe("**bold**")
  })

  test("bold via b", () => {
    expect(htmlToMd(el("<b>bold</b>"))).toBe("**bold**")
  })

  test("italic via i", () => {
    expect(htmlToMd(el("<i>italic</i>"))).toBe("_italic_")
  })

  test("emphasis via em is highlight", () => {
    expect(htmlToMd(el("<em>highlight</em>"))).toBe("==highlight==")
  })
})

describe("links", () => {
  test("basic link", () => {
    expect(htmlToMd(el('<a href="https://example.com">Example</a>'))).toBe(
  ...
  test("link with title", () => {
    expect(
      htmlToMd(el('<a href="https://example.com" title="Example Site">Example</a>')),
    ).toBe('[Example](https://example.com "Example Site")')
  })

  test("link without href is plain text", () => {
    expect(htmlToMd(el("<a>Nothing</a>"))).toBe("Nothing")
  })
})

describe("images", () => {
  test("basic image", () => {
    expect(htmlToMd(el('<img src="image.png" alt="Alt">'))).toBe("![Alt](image.png)")
  })

  test("image with title", () => {
    expect(
      htmlToMd(el('<img src="image.png" alt="Alt" title="Photo">')),
    ).toBe('![Alt](image.png "Photo")')
  })

  test("image without src is skipped", () => {
    expect(htmlToMd(el('<img alt="Alt">'))).toBe("")
  })
})

describe("inline code", () => {
  test("basic code", () => {
    expect(htmlToMd(el("<code>code</code>"))).toBe("`code`")
  })
  ...
  test("code with leading backtick", () => {
    expect(htmlToMd(el("<code>`test</code>"))).toBe("`` `test ``")
  })
})

describe("horizontal rule", () => {
  test("hr", () => {
    expect(htmlToMd(el("<hr>"))).toBe("---")
  })
})

describe("unordered lists", () => {
  test("basic ul", () => {
    expect(htmlToMd(el("<ul><li>One</li><li>Two</li></ul>"))).toBe("- One\n- Two")
  })
})

describe("ordered lists", () => {
  test("basic ol", () => {
    expect(htmlToMd(el("<ol><li>First</li><li>Second</li></ol>"))).toBe("1.  First\n2.  Second")
  })
})

describe("blockquotes", () => {
  test("basic blockquote", () => {
    expect(htmlToMd(el("<blockquote><p>Quote</p></blockquote>"))).toBe("> Quote")
  })
})
...
describe("code blocks", () => {
  ...
  test("pre code without language", () => {
    expect(htmlToMd(el("<pre><code>let x = 1;</code></pre>"))).toBe("```\nlet x = 1;\n```")
  })
...
describe("code-by feature", () => {
  ...
  test("code-by no match when class absent", () => {
    expect(htmlToMd(el("<h3>plain heading</h3>"), { codeBy: ["h3.property"] })).toBe("### plain heading")
  })
})
...
describe("figcaption and address", () => {
  test("figcaption becomes italic", () => {
    expect(htmlToMd(el("<figcaption>Caption</figcaption>"))).toBe("_Caption_")
  })

  test("address becomes bold", () => {
    expect(htmlToMd(el("<address>Author</address>"))).toBe("**Author**")
  })
})

describe("form and fieldset are skipped", () => {
  test("form is skipped", () => {
    expect(htmlToMd(el("<form>content</form>"))).toBe("")
  })

  test("fieldset is skipped", () => {
    expect(htmlToMd(el("<fieldset>content</fieldset>"))).toBe("")
  })
})

describe("interactive elements are skipped", () => {
  test("button is skipped", () => {
    expect(htmlToMd(el("<button>Click</button>"))).toBe("")
  })

  test("input is skipped", () => {
    expect(htmlToMd(el('<input type="text" value="hello">'))).toBe("")
  })

  test("select is skipped", () => {
    expect(htmlToMd(el("<select><option>A</option></select>"))).toBe("")
  })

  test("textarea is skipped", () => {
    expect(htmlToMd(el("<textarea>text</textarea>"))).toBe("")
  })
})

describe("es carp ing", () => {
  test("asterisks escaped", () => {
    expect(htmlToMd(el("<p>3 * 4 = 12</p>"))).toBe("3 \\* 4 = 12")
  })

  test("underscores escaped", () => {
    expect(htmlToMd(el("<p>_hello_</p>"))).toBe("\\_hello\\_")
  })

  test("backticks escaped", () => {
    expect(htmlToMd(el("<p>`code`</p>"))).toBe("\\`code\\`")
  })

  test("inline code is not escaped", () => {
    expect(htmlToMd(el("<p>inline <code>_code_</code> here</p>"))).toBe(
      "inline `_code_` here",
    )
  })

  test("code blocks are not escaped", () => {
    expect(htmlToMd(el("<pre><code>*bold*</code></pre>"))).toBe("```\n*bold*\n```")
  })
})

describe("whitespace", () => {
  test("multiline text collapses", () => {
    expect(htmlToMd(el("<p>Hello\n\nWorld</p>"))).toBe("Hello World")
  })
  ...
  test("whitespace preserved in pre", () => {
    expect(htmlToMd(el("<pre><code>  spaced  </code></pre>"))).toBe("```\n  spaced  \n```")
  })
})

describe("post-processing", () => {
  test("hash-link anchors stripped", () => {
    expect(htmlToMd(el('<a href="#section">​</a>'))).toBe("")
  })
})
...
describe("image hoisting", () => {
  ...
  test("default flags does not hoist", () => {
    expect(htmlToMd(el('<img src="cat.png" alt="Cat">'))).toBe("![Cat](cat.png)")
  })
})
...
describe("skip flags", () => {
  ...
  test("header skipped with SkipFlags.HEADER", () => {
    expect(htmlToMd(el("<header>Brand</header><p>Body</p>"), { skip: SkipFlags.HEADER })).toBe("Body")
  })

  test("footer skipped with SkipFlags.FOOTER", () => {
    expect(htmlToMd(el("<p>Body</p><footer>Copyright</footer>"), { skip: SkipFlags.FOOTER })).toBe("Body")
  })

  test("aside skipped with SkipFlags.ASIDE", () => {
    expect(htmlToMd(el("<aside>Sidebar</aside><p>Main</p>"), { skip: SkipFlags.ASIDE })).toBe("Main")
  })

  test("nav skipped with SkipFlags.NAV", () => {
    expect(htmlToMd(el("<nav>Links</nav><p>Content</p>"), { skip: SkipFlags.NAV })).toBe("Content")
  })

  test("menu skipped with SkipFlags.MENU", () => {
    expect(htmlToMd(el("<menu><li>Item</li></menu><p>Body</p>"), { skip: SkipFlags.MENU })).toBe("Body")
  })

  test("tags preserved without skip flag", () => {
    expect(htmlToMd(el("<header>Brand</header><p>Body</p>"))).toBe("Brand\n\nBody")
  })
