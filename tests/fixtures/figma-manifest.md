...
Used to specify HTML file/files that can be used in the iframe modal via [`figma.showUI`][ref0], if you choose to have one.

- If a single string is specified, this is the relative file path to the HTML file whose contents will be available as a string in the Javascript code via the constant [`__html__`][ref1].
- If a map is specified, each entry of the map will be available at [`__uiFiles__`][ref2]
...
If your existing plugin's manifest doesn't include this field, you should [migrate your plugin to use dynamic page loading][ref3].
...
- A `key` property that is a unique id used to identify the value of this property in [`ParameterValues`][ref5].
...
Allows you to specify that your plugin can only be launched with parameters. You must also specify a valid parameter list: [`parameters`][ref6]. Can also be defined on [menu items][ref4] with parameters.
...
This allows you to specify a submenu for your plugin, enabling the plugin to contain multiple commands. Use the [`figma.command`][ref7] property to determine which command the user selected.
...
This configures the relaunch buttons that are set via the [`setRelaunchData`][ref8] API call.
...
- A `command` property that specifies the `figma.command` that will be supplied when the plugin is run after the button is pressed. This command must match the command supplied in the [`setRelaunchData`][ref8] API call for the button to display.
...
See [Proposed API][ref9]. **Warning: This flag is only meant for development, and will not work in published plugins\!**
...
> [!NOTE]
> 
> - `activeusers`: lets your plugin use the [`figma.activeUsers`][ref10] API.
> - `currentuser`: lets your plugin use the [`figma.currentUser`][ref11] API.
> - `fileusers`: lets you use the [`StampNode.getAuthorAsync`][ref12] method.
> - `payments`: lets you use the [`figma.payments`][ref13] API.
> - `teamlibrary`: lets you use the [`figma.teamLibrary`][ref14] API.
...
Use `textreview` for [text review plugins][ref15].

Use `codegen` for [codegen plugins][ref16].

Use `inspect` for [inspect panel plugins][ref17].

Use `vscode` for [Dev Mode plugins in VS Code][ref18].

---

### `codegenLanguages?: `[CodeLanguage][ref19]`[]`
...
### `codegenPreferences?: `[CodegenPreference][ref20]`[]`
...
[ref0]: /docs/plugins/api/properties/figma-showui/
[ref1]: /docs/plugins/api/global-objects/#html
[ref2]: /docs/plugins/api/global-objects/#uifiles
[ref3]: /docs/plugins/migrating-to-dynamic-loading/
[ref4]: /docs/plugins/manifest/#menu
[ref5]: /docs/plugins/api/figma-parameters/#parametervalues
[ref6]: /docs/plugins/manifest/#parameters
[ref7]: /docs/plugins/api/figma/#command
[ref8]: /docs/plugins/api/properties/nodes-setrelaunchdata/
[ref9]: /docs/plugins/proposed-api/
[ref10]: /docs/plugins/api/figma/#activeusers
[ref11]: /docs/plugins/api/figma/#currentuser
[ref12]: /docs/plugins/api/StampNode/#getauthorasync
[ref13]: /docs/plugins/api/figma/#payments
[ref14]: /docs/plugins/api/figma/#teamlibrary
[ref15]: /docs/plugins/textreview-plugins/
[ref16]: /docs/plugins/working-in-dev-mode/#plugins-for-codegen
[ref17]: /docs/plugins/working-in-dev-mode/#plugins-for-inspection
[ref18]: /docs/plugins/working-in-dev-mode/#dev-mode-plugins-in-visual-studio-code
[ref19]: /docs/plugins/api/CodeLanguage/
[ref20]: /docs/plugins/api/CodegenPreference/
